...
│           Skill refactor — load on demand            │
├─────────────────────────────────────────────────────┤
│  Layer 4  Subagents — 重活隔离出去，只回收结果          │
│           Subagents — isolate heavy work, return summaries│
├─────────────────────────────────────────────────────┤
│  Layer 3  Hooks — 规范检查交给代码，不靠 AI 记          │
│           Hooks — code-enforced checks, not memory   │
├─────────────────────────────────────────────────────┤
│  Layer 2  Auto Memory — 跨会话自动积累经验和踩坑        │
│           Auto Memory — cross-session knowledge accrual│
├─────────────────────────────────────────────────────┤
│  Layer 1  CLAUDE.md — 约束写死，compact 后重新注入      │
│           CLAUDE.md — persisted constraints, re-injected│
└─────────────────────────────────────────────────────┘
```
